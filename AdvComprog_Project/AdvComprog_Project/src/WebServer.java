...
public class WebServer {
        ...
        server.createContext("/calculate", (exchange) -> {
            try {
                if ("POST".equals(exchange.getRequestMethod())) {
                    ...
                    double svtotal = bill.getTotal();

                    SplitStrategy splitStrategy = new EqualSplit();
                    double[] split = splitStrategy.split(svtotal, people);

                    // Response with Aesthetic Receipt
                    String response = getReceipt(total, svtotal, split[0], people);
                    byte[] responseBytes = response.getBytes(StandardCharsets.UTF_8);
    ...
    static String getForm() {
        return "<html><head><meta charset='UTF-8'>" +
                ...
                "    <div class='plate-grid'>" +
                        ...
                        plateRow("Gold", "plate_gold_pattern", "#8f8249") +
                "    </div>" +
    ...
    static String getReceipt(double total, double svtotal, double split, int people) {
        return "<html><head><meta charset='UTF-8'>" +
                "<style>" +
                ...
                "    <div class='line'><span class='promo'>Service Charge 10%</span><span class='promo'>+฿" + String.format("%.2f", svtotal - total) + "</span></div>" +
